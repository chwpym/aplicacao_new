    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if items:
                    # Print interesting parts of the first item
                    item = items[0]
                    summary = {
                        "brand": item.get("brand", {}).get("value"),
                        "model": item.get("model", {}).get("value"),
                        "year": item.get("year", {}).get("value"),
                        "cross": [
                            x.get("value")
                            for x in item.get("crossReference", {}).get("valueList", [])
                        ][:3],
                    }
                    print(f"Sample data: {summary}")
                else:
                    print("Catalog is empty for this code.")
            else:
