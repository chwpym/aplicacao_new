...
async def sync_catalog():
    print(f"[{datetime.now().isoformat()}] Iniciando Sincronização FIPE (Parallelum API V2)...")
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        try:
            ...
            for i, marca in enumerate(marcas_fipe):
                marca_nome = marca.get("name", "").upper()
                marca_id = marca.get("code")
                ...
                try:
                    url_modelos = f"https://fipe.parallelum.com.br/api/v2/cars/brands/{marca_id}/models"
