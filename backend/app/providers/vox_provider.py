...
class VoxProvider(BaseProvider):
    def __init__(self, config: Dict[str, Any]):
        self.config = config
        self.base_url = "https://www.filtrosvox.com.br/VoxCatalogo"
    ...
    async def _descobrir_imagens(self, fallback_cod: str) -> List[str]:
        base_url = "https://www.tecfil.com.br/imagens/vox"
        candidatos = [
    ...
    async def buscar(self, codigo: str) -> List[Dict[str, Any]]:
        ...
        try:
            # 1. Inicia Sessão e faz o POST de busca usando form j_idt67 da Vox
            res_index = await self.client.get(f"{self.base_url}/catalogo.xhtml")
            soup_index = BeautifulSoup(res_index.text, "html.parser")

            form_index = soup_index.find("form", id="j_idt67") or soup_index.find("form")
            vs_input = soup_index.find("input", {"id": "j_id1:javax.faces.ViewState:2"}) or (form_index.find("input", {"name": "javax.faces.ViewState"}) if form_index else None)
            ...
            res_search = await self.client.post(f"{self.base_url}/catalogo.xhtml", data=data_index)
            ...
            for btn in buttons:
                val = btn.get("value", "")
                match = re.search(r"Total de Resultados\s*\[\s*(\d+)\s*\]", val)
            ...
            if conv_cat:
                print(f"[VOX] Extraindo Conversões para {codigo_buscado}...")
            ...
            for cat in categories_to_fetch:
                print(f"[VOX] Extraindo {cat['label']} para {codigo_buscado}...")
                ...
                async def parse_table_rows(html_content, is_soup=False):
                    ...
                    for r in trows:
                        ...
                        if len(cells) >= 7:
                            ...
                            # Vox product image url format (hosted on Tecfil website)
                            imagens_validas = await self._descobrir_imagens(codigo_buscado)
                            imagem_url = imagens_validas[0] if imagens_validas else f"https://www.tecfil.com.br/imagens/vox/{codigo_buscado}A.jpg"
            ...
            print(f"[VOX] Erro crítico no provider: {str(e)}")
            return []
