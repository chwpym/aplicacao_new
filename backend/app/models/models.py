...
class Provedor(Base):
    ...
    url = Column(String)
    tipo = Column(String) # graphql, rest, ds, ate, pdf_local, viemar, schadek, iguacu, mte_thomson
    ...
    mapeamento = Column(Text, nullable=True) # JSON com mapeamento de campos (REST) ou seletores (Scraper)

class Sigla(Base):
    __tablename__ = "siglas"

    id = Column(Integer, primary_key=True, index=True)
    nome_completo = Column(String, unique=True, index=True)
