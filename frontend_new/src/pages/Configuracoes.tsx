...
const STORAGE_KEY = 'visibleFieldsDefault';
...
const Configuracoes = () => {
    ...
    useEffect(() => {
        try {
            ...
            if (raw) {
                setFields({ ...factoryDefaults, ...JSON.parse(raw) });
            } else {
        ...
        <div className="max-w-3xl mx-auto space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-slate-500 text-sm">Defina quais colunas ficam visíveis por padrão ao abrir o sistema.</p>
                </div>
            ...
            <div className="flex items-center justify-between bg-slate-50 dark:bg-slate-900/50 rounded-xl px-5 py-3 border border-slate-200 dark:border-slate-800">
                <span className="text-sm font-semibold text-slate-500">
                    <span className="text-primary font-black">{activeCount}</span> de {allFields.length} colunas ativas
                </span>
            ...
            {/* Nota */}
            <div className="text-xs text-slate-400 text-center pt-4 font-medium">
                As preferências são salvas localmente no seu navegador. Na tela de busca você ainda pode alterar temporariamente.
            </div>
        </div>
    );
};
