        ...
        <div className="md:block hidden overflow-x-auto custom-scrollbar">
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-50 dark:bg-slate-800/40 text-slate-500 uppercase text-[10px] font-bold tracking-wider">
              <tr>
                {visibleFields.marca && (
                  <th className="px-6 py-2">{getFieldLabel("marca")}</th>
                )}
                {visibleFields.veiculo && (
                  <th className="px-6 py-2">{getFieldLabel("veiculo")}</th>
                )}
                {visibleFields.modelo && (
                  <th className="px-6 py-2">{getFieldLabel("modelo")}</th>
                )}
                {visibleFields.versao && (
                  <th className="px-6 py-2">{getFieldLabel("versao")}</th>
                )}
                {visibleFields.motor && (
                  <th className="px-6 py-2">{getFieldLabel("motor")}</th>
                )}
                ...
                {visibleFields.observacao && (
                  <th className="px-6 py-2">{getFieldLabel("observacao")}</th>
                )}
                {visibleFields.posicao && (
                  <th className="px-6 py-2">{getFieldLabel("posicao")}</th>
                )}
                {visibleFields.lado && (
                  <th className="px-6 py-2">{getFieldLabel("lado")}</th>
                )}
                {visibleFields.direcao && (
                  <th className="px-6 py-2">{getFieldLabel("direcao")}</th>
                )}
                ...
                {visibleFields.restricao && (
                  <th className="px-6 py-2">{getFieldLabel("restricao")}</th>
                )}
                {visibleFields.apenas && (
                  <th className="px-6 py-2">{getFieldLabel("apenas")}</th>
                )}
                {visibleFields.referencias && (
                  <th className="px-6 py-2">{getFieldLabel("referencias")}</th>
                )}
                {visibleFields.imagem && (
                  <th className="px-6 py-2">{getFieldLabel("imagem")}</th>
                )}
              </tr>
            </thead>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {res.imagens && res.imagens.length > 0 ? (
                            ...
                            <div className="h-10 w-10 rounded-lg bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-400">
                              <div className="opacity-20 text-[9px] font-bold">
                                N/A
                              </div>
                            </div>
                          )}
