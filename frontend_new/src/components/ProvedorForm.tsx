...
import { X, Save, Shield, Database, Globe, RefreshCcw, HelpCircle, Search, FlaskConical } from 'lucide-react';
...
const ProvedorForm: React.FC<ProvedorFormProps> = ({ provedor, onSave, onCancel }) => {
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
        ...
        setFormData((prev: any) => {
            ...
            if (name === 'nome' || name === 'tipo') {
                const currentNome = name === 'nome' ? String(value).toUpperCase() : (next.nome || '').toUpperCase();
                ...
                if (currentNome === 'DS') {
                    if (currentTipo === 'rest') {
                        ...
                        // Sugere a URL de busca global da DS
            ...
            <div className="bg-surface-light dark:bg-surface-dark w-full max-w-2xl rounded-2xl shadow-2xl flex flex-col max-h-[90vh] animate-in fade-in zoom-in duration-200">
                ...
                <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-xl font-bold">{provedor ? 'Editar Provedor' : 'Novo Provedor'}</h2>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-2">
                            <label className="text-sm font-semibold text-slate-500 uppercase tracking-wider">Nome</label>
                            <input
                        ...
                        <div className="space-y-2">
                            ...
                            >
                                <option value="graphql">GraphQL (Fraga/SABÓ/Authomix)</option>
                                ...
                                <option value="ate">Catálogo ATE</option>
                            </select>
                        </div>
                    </div>

                    <div className="space-y-2">
                        <label className="text-sm font-semibold text-slate-500 uppercase tracking-wider">URL do Endpoint</label>
                        <input
                    ...
                    {formData.tipo === 'graphql' && (
                        <div className="space-y-2">
                            <div className="flex justify-between items-center">
                                <label className="text-sm font-semibold text-slate-500 uppercase tracking-wider">Query GraphQL</label>
                                <button onClick={handleRestoreTemplate} className="text-[10px] text-primary hover:underline flex items-center gap-1">
                    ...
                    {(formData.tipo === 'rest' || formData.tipo === 'ds' || formData.tipo === 'scraper') && (
                        <div className="space-y-4 border-t border-slate-200 dark:border-slate-800 pt-4">
                            <div className="flex justify-between items-center">
                                <label className="text-sm font-semibold text-slate-500 uppercase tracking-wider">
                                    {formData.tipo === 'rest' ? 'Mapeamento JSON' : 'Seletores CSS (Scraper)'}
                                </label>
                                <span className="text-[10px] text-slate-400">Define como extrair os dados</span>
                            </div>
                            <div className="grid grid-cols-2 gap-3">
                                {['container', 'product_link', 'marca', 'veiculo', 'modelo', 'motor', 'configuracao_motor', 'ano_inicio', 'ano_fim', 'imagem', 'image_pattern', 'referencias', 'ref_marca', 'ref_codigo', 'observacao'].map(field => (
                                    <div key={field} className="space-y-1">
                                        <label className="text-[10px] text-slate-400 upper-case font-bold">
                                            {(() => {
                                                ...
                                                return labels[field] || field.toUpperCase().replace('_', ' ');
                                            })()}
                                        </label>
                                        <input
                                            placeholder={(formData.tipo === 'rest' || formData.tipo === 'scraper') ? `Ex: ${field}` : `CSS: .${field}`}
                                            ...
                                            onChange={(e) => {
                                                ...
                                                setFormData((prev: any) => {
                                                    let currentMap = {};
                                                    try { currentMap = JSON.parse(prev.mapeamento); } catch { }
                                                    const nextMap = { ...currentMap, [field]: val };
                                                    return { ...prev, mapeamento: JSON.stringify(nextMap) };
                                                });
                                            }}
                                        />
                                    </div>
                    ...
                    <div className="space-y-4 pt-4 border-t border-slate-200 dark:border-slate-800">
                        <div className="flex items-center justify-between">
                            ...
                            <input
                                type="checkbox" name="login_required"
                                checked={formData.login_required} onChange={handleChange}
                        ...
                        {formData.login_required && (
                            <div className="grid grid-cols-2 gap-4 animate-in slide-in-from-top-2 duration-300">
                                <div className="space-y-2">
                                    <label className="text-sm font-semibold text-slate-500 uppercase tracking-wider">E-mail / Usuário</label>
                                    <input
                                ...
                                <div className="space-y-2">
                                    <label className="text-sm font-semibold text-slate-500 uppercase tracking-wider">Senha</label>
                                    <input
                    ...
                    <div className="flex items-center justify-between pt-4">
                        <div className="flex items-center gap-2">
                            <div className={`w-3 h-3 rounded-full ${formData.ativo ? 'bg-emerald-500' : 'bg-slate-300'}`} />
            ...
            {showHelp && (
                <div className="fixed inset-y-0 right-0 w-80 bg-surface-light dark:bg-surface-dark border-l border-slate-200 dark:border-slate-800 shadow-2xl z-[110] p-8 animate-in slide-in-from-right duration-300 custom-scrollbar overflow-y-auto">
                    <div className="flex justify-between items-center mb-8">
                        ...
                        <button onClick={() => setShowHelp(false)} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full">
                            <X size={20} />
                        </button>
                    </div>
                    <div className="space-y-6 text-sm text-slate-500 leading-relaxed text-justify">
                        <section>
                            ...
                            <p>Para catálogos que usam a tecnologia da Fraga (SABÓ, INDISA, AuthOmix), selecione o tipo **GraphQL**. A query será preenchida automaticamente.</p>
                        </section>
                        <section>
                            <h4 className="text-slate-900 dark:text-slate-100 font-bold mb-2 flex items-center gap-2">
                                <Search size={16} className="text-primary" /> REST API / ERP
                            </h4>
                            <p>Para buscar dados de um ERP ou servidor REST, use o **Mapeamento JSON**. Basta dizer ao sistema qual campo da API deles corresponde aos nossos campos (ex: `brand` &rarr; `Marca`).</p>
                        </section>
                        <section>
                            <h4 className="text-slate-900 dark:text-slate-100 font-bold mb-2 flex items-center gap-2">
                                <Database size={16} className="text-primary" /> Robô Scraper
                            </h4>
                            <p>Se o site não tem API, o robô Scraper pode ler os dados visualmente usando **seletores CSS**. É como dizer: "Olhe para a tabela e pegue o texto da primeira coluna".</p>
                        </section>
                        <section>
                            <h4 className="text-slate-900 dark:text-slate-100 font-bold mb-2 flex items-center gap-2">
                                <RefreshCcw size={16} className="text-primary" /> Sugestões
                            </h4>
                            <p>Ao digitar nomes como **DS**, o sistema tentará preencher o mapeamento ideal para você automaticamente.</p>
                        </section>
                    </div>
                </div>
            )}
